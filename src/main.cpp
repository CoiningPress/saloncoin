...
unsigned int nStakeMinAge = 60 * 60 * 24 * 1;	// minimum age for coin age: 2d
unsigned int nStakeMaxAge = 60* 60 * 24 * 180;	// stake age of full weight: -1
unsigned int nStakeTargetSpacing = 30;			// 30 sec block spacing

int64 nChainStartTime = 1399298400;
...
const string strMessageMagic = "SalonCoin Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in SalonCoin
    bool fStrictPayToScriptHash = true; // Always active in SalonCoin
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    printf( "nStakeModifierChecksum %08X \n",pindexNew->nStakeModifierChecksum );
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SalonCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = 1399298200;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1421918;
        if (false ) {
        ...
        assert(block.hashMerkleRoot == uint256("f15d6bd6e124d50d3fc8fc8e35563f45e701113ec45684b9172e1fcd2bdf1569"));
